...
type Props = {
  ...
  info: ModulosPerfiles[];
  setPerfiles: React.Dispatch<React.SetStateAction<ModulosPerfiles[]>>;
  setInfoEditar: React.Dispatch<React.SetStateAction<ModulosPerfiles>>;
};
const Table = ({ info, setPerfiles, setInfoEditar, setShowModal }: Props) => {
  const columns: any = [
    {
      name: "Perfil",
    ...
    {
      name: "Rol",
      selector: (row: ModulosPerfiles) =>
        (row.Tipo == 1 && "COLABORADOR") ||
        (row.Tipo == 2 && "PROFESOR") ||
        (row.Tipo == 3 && "ESTUDIANTE") ||
        (row.Tipo == 4 && "ACUDIENTE"),
      sortable: true,
      wrap: true,
    },
    {
              ...
              if (validate) {
                ...
                try {
                  const responseRemove = await fetch(
                    "/api/Seguridad/DeleteModulosPerfil",
                    {
                      method: "DELETE",
                      headers: {
