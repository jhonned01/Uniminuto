...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [ingreso]: any = await connectionPool.query(
      `INSERT INTO preguntas_pruebas(tipo,pregunta,opciones,respuesta,punto,competencia,prueba,aprobo,IdDocente) VALUES(2, '${pregunta}', '${respuestasFormated}','${correcta}', ${punto}, '${competenciaFind[0]?.eje_id}', '${prueba}', 0,'${IdUser}')`
    );
